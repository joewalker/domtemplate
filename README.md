...
unreleased) Firefox developer tools, so it should be well supported and well
used.
...
* There are a number of areas (noted in the docs) where we've not done enough
  testing. This is an alpha after all.
...
Example:
    ...
    <div><p if="${name !== null}>Hi, ${name}</p></div>
...
* 2-way templating. For form fill applications, it can be useful to allow
  ...
  This example is probably too complex for our purposes however.
