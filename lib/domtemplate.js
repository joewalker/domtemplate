...
Templater.prototype._processForEachInner = function(template, sibling, data) {
  if (sibling.nodeName.toLowerCase() === 'loop') {
    ...
    parts.forEach(function(part) {
      ...
      if (typeof part.then === 'function') {
        ...
        part.then(function(reply) {
          reply = this._toNode(reply, tempNode.ownerDocument);
        ...
        thing = this._toNode(thing, node.ownerDocument);
        node.parentNode.insertBefore(thing, node);
      }
    }, this);
    node.parentNode.removeChild(node);
  }
